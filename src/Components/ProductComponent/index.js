...
const ProductComponent = () => {
    ...
    const getProducts = async () => {
        try {
            const response = await axios.get(`https://my-fresh-follow-app.onrender.com/freshFollow/product/getOne/${id}`, {
                headers: {
    ...
    if (!Object.keys(products).length > 0) return <div> Product Not Found ☹️ </div>
                ...
                <section className="text-gray-600 body-font overflow-hidden">
                    <div className="container px-5 py-24 mx-auto">
                        <div className="lg:w-4/5 mx-auto flex flex-wrap">
                            <img alt={products?.title} className="lg:w-1/2 w-full lg:h-auto max-h-[600px] h-64 object-contain object-center rounded" src={products?.image} />
                                ...
                                <div className="flex">
                                    ...
                                    <div className='ml-4 flex justify-between items-center'>
                                        <StripeCheckout name="Payment" currency="INR" token={makePayment} stripeKey="pk_test_51MlnPYSDphT43pOnf3L4Dh9Z8KudUcu3CEECHgznzTj8cojvmlU4ft1GlOwVIZbqR164Gj4wWUQrmOuE3daiaTHG00O5c5DGv7">
                                            <button className='flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded mr-2'>Buy Now</button>
                                        </StripeCheckout>
                                        <button className="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded mr-2" onClick={() => handleCart(products)}>Add to Cart</button>
                                    </div>
